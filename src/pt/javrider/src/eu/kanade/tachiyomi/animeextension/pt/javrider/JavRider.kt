...
class JavRider : ParsedAnimeHttpSource() {
    ...
    private val fields = "id,title,link,_embedded,_links"
    private val embed = "wp:featuredmedia"

    // ==================== LISTAGEM ====================

    override fun popularAnimeRequest(page: Int): Request {
        val url = "$apiUrl/posts?per_page=24&page=$page&_embed=$embed&_fields=$fields"
    ...
    private fun extractFromPlayer(iframeUrl: String, referer: String): List<Video> {
        ...
        try {
            ...
            val req = Request.Builder().url(iframeUrl).headers(playerHeaders).build()
            val html = client.newCall(req).execute().use { it.body!!.string() }

            // Padrão principal: https://javplayers.com/m3/{base64}
            ...
            // Regex secundária: m3u8/mp4 soltos
            if (videos.isEmpty()) {
            ...
            // silencioso: retornamos o que já temos
        }
        return videos
    }
